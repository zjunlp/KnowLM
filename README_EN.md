...
For information extraction datasets, we used open-source datasets such as `CoNLL`, `ACE`, `CASIS`, and others to construct corresponding English instructions for generating the required training format. For the Chinese part, for NER and EE tasks, we utilized open-source datasets such as `DualEE`, `PEOPLE DAILY`, and others, and then created corresponding Chinese instructions to synthesize the required training format. As for the RE task, we sampled a subset of instructions from the `DuIE`, `DuIE2.0`, and a dataset we constructed called [KG2Instruction](https://arxiv.org/abs/2305.11527). Specifically, we used Chinese Wikipedia data and employed entity linking with the help of crowdworkers and the Hanlp toolkit to identify Chinese entities. We then mapped these entities to their corresponding identifiers (Qids) in the Wikidata. However, since there may be ambiguities (i.e., a Chinese entity can have multiple identifiers, such as "apple" which can refer to both a fruit and a company), we designed a strategy to disambiguate them. Next, we used a distant supervision method to generate possible triples based on the identified entities. We applied predefined rules to filter out invalid or incorrect triples. Finally, we constructed the corresponding Chinese instructions to generate the required training format.
