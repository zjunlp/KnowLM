...
- ❗请注意本项目仍在迭代优化中，模型权重会定期更新，并正在上传百度网盘和谷歌云盘存储的权重！
...
| -------------- | ------------ | --------------------- | -------- | ---------------------------------- | ------------------------------------------------------------ |
| ZhiXi-13B-Diff | 全量预训练   | 与LLaMA-13B作差的权重 | 46GB     | [HuggingFace](https://huggingface.co/zjunlp/zhixi-13b-diff) <br/> [百度网盘]() <br/> [谷歌云盘]() | 恢复预训练的权重（即ZhiXi-13B）需要搭配LLaMA-13B的权重，具体使用说明请参考[此处](#2-2) |
| ZhiXi-13B-LoRA | LoRA指令微调 | LoRA权重              | 251MB    | [HuggingFace](https://huggingface.co/zjunlp/zhixi-13b-lora) <br/> [百度网盘]() <br/> [谷歌云盘]()  | 需要搭配ZhiXi-13B进行使用，具体使用说明参考[此处](#2-4)          |
