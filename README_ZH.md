...
- ❗**如果您在安装和使用KnowLM中遇到任何问题（一般是包的版本兼容性、速度或硬件配置问题）不用心急，您可以直接提Issue，我们会尽全力帮助您解决问题**！
...
> [第2节](#2)中的例子均在V100上进行推理，如若在其他设备上进行推理，结果可能有出入，请多次运行或修改解码参数。**对于`knowlm-13b-zhixi`和`knowlm-13b-ie`，我们是基于`knowlm-13b-base`使用lora训练得到的，`knowlm-13b-zhixi`和`knowlm-13b-ie`是我们将训练得到的lora权重和`knowlm-13b-base`权重合并后的模型参数。**
